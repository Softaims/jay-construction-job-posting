import { fetchPendingContractors, updateAdminStatusDocumentVerification, updateAdminStatusToPending } from "./services.js";
...
export const submitContractorDocuments = catchAsync(async (req, res, next) => {
  ...
  const [complianceUploadResult, verificationUploadResult] = await Promise.all([s3Uploader(complianceFile), s3Uploader(verificationFile)]);
...
export const getPendingContractors = catchAsync(async (req, res, next) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 10;

  const { pendingContractors, total } = await fetchPendingContractors(page, limit);
